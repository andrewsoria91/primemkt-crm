...
use DateTime;
use Iterator;
use GO\Base\Util\Icalendar\DateTimeParser;
...
class RRuleIterator implements Iterator {
	 ...
	 * @param int $periodEndTime unixtime
	 * @return int timestamp or false then end time excided
	 */
	public function nextRecurrence($periodEndTime=0) {
		$occurence = $this->current(); //$this->next();
		if($occurence !== null && $occurence->getTimestamp() > $periodEndTime) {
			return false;
		}
		$this->next();
		if($occurence !== null) {
			return $occurence->getTimestamp();
		}
		return false;
	}

	/**
	 * Added for backward compatible with toHtml() of Event to displat rule as text
	 * @return string html
	 */
	public function getAsText() {
		...
		if(!empty($this->byDay)){
			foreach($this->byDay as $icalDay){
				...
				$index = array_search(substr($icalDay, -2), array('SU','MO','TU','WE','TH','FR','SA'));
				$days[]=$fulldays[$index];
			}
		}
     ...
     */
    function __construct($rrule, $start) {
     ...
     * @return void
     */
    function fastForward(DateTime $dt) {
        ...
        if (empty($this->byMonth)) {
            ...
            if ($this->byYearDay !== null) { // byYearDay is an array with values from -366 to -1, or 1 to 366
                ...
                if ($this->byDay) {
                    ...
                    $dayOffsets = array(1,2,3,4,5,6,7);
                }
            ...
            switch ($key) {
                ...
                case 'COUNT' :
                    $val = (int)$value;
										...
										// In the old version of GO, an empty "INTERVAL" was accepted 
										// and processed as 1. 
										// (Eg: RRULE:FREQ=YEARLY;INTERVAL= was processed as: RRULE:FREQ=YEARLY;INTERVAL=1)
										if(empty($val)){
											$val = 1;
										}
                ...
                case 'BYYEARDAY' :
                    ...
                    foreach ($this->byYearDay as $byYearDay) {
                        if (!is_numeric($byYearDay) || (int)$byYearDay < -366 || (int)$byYearDay == 0 || (int)$byYearDay > 366) {
                            throw new \InvalidArgumentException('BYYEARDAY in RRULE must have value(s) from 1 to 366, or -366 to -1!');
                        }
                    }
                    break;
                ...
                case 'BYWEEKNO' :
                    ...
                    foreach ($this->byWeekNo as $byWeekNo) {
                        if (!is_numeric($byWeekNo) || (int)$byWeekNo < -53 || (int)$byWeekNo == 0 || (int)$byWeekNo > 53) {
                            throw new \InvalidArgumentException('BYWEEKNO in RRULE must have value(s) from 1 to 53, or -53 to -1!');
                        }
                    }
                    break;
                ...
                case 'BYMONTH' :
                    ...
                    foreach ($this->byMonth as $byMonth) {
                        if (!is_numeric($byMonth) || (int)$byMonth < 1 || (int)$byMonth > 12) {
                            throw new \InvalidArgumentException('BYMONTH in RRULE must have value(s) betweeen 1 and 12!');
                        }
                    }
                    break;
