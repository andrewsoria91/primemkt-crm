 ...
 * @version $Id: TimeTrackingReport.php 21849 2016-11-08 12:46:37Z mschering $ 
...
class TimeTrackingReport extends AbstractReport {
	...
	public function name() {
		return \GO::t("timeEntries","projects2");
	}
...
//			\GO\Base\Util\Http::outputDownloadHeaders(new \GO\Base\Fs\File($this->filename.".pdf"));
	...
	private function _addTimeEntries($project=null) {
		...
		$titleFn='h2';
		$title = sprintf(GO::t('timeEntries','projects2'), GO::t('allProjects','projects2'));
		
		//query all subprojects of given path
		if($this->project){
			$titleFn='h2';
			$title = sprintf(GO::t('timeentriesPdfReportTitle','projects2'), $this->project->path);
...
//			$fp->getCriteria()->addCondition('project_id', $this->project->id);
		...
		$html .= '<thead>' .
						$this->_pdf->tableHeaders(array(
								...
								new \GO\Base\Util\PdfTableColumn(array('width' => 50,'text' => \GO::t('strHours'), 'class'=>'head', 'align' => 'right')),
